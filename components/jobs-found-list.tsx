...
import { ArrowUpRight, BriefcaseBusiness, Check, Clock3, Inbox, MapPin, RotateCcw, Trash2 } from "lucide-react"
...
export function JobsFoundList({ initialJobs }: { initialJobs: FoundJob[] }) {
  ...
  return <div className="space-y-6">
    ...
    {showDeleted ? <DeletedList jobs={deleted} pendingId={pendingId} moveJob={moveJob} /> : <>
      {featured ? <Card className="gap-0 overflow-hidden border-primary/30 py-0 shadow-sm">
        <CardHeader className="bg-muted/40 p-6">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0 space-y-2">
              <p className="text-sm font-semibold uppercase tracking-wider text-primary">{featured.company}</p>
              <CardTitle className="text-2xl leading-tight sm:text-3xl">{featured.position}</CardTitle>
              {featured.location && <p className="flex items-center gap-1.5 text-sm text-muted-foreground"><MapPin className="size-4" />{featured.location}</p>}
            </div>
            <div className="flex shrink-0 flex-wrap gap-2"><Badge variant="secondary">{featured.role_type === "internship" ? "Internship" : "Full time"}</Badge><Badge>{feed === "jobs_found" ? "New" : "Snoozed"}</Badge></div>
          </div>
        </CardHeader>
        <CardContent className="space-y-6 border-t p-6">
          <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            <Detail label="Location" value={featured.location || "Not provided"} />
...
function DeletedList({ jobs, pendingId, moveJob }: { jobs: FoundJob[]; pendingId: string | null; moveJob: (id: string, status: AiJobStatus | "to_apply") => Promise<void> }) {
  ...
  return <section className="space-y-3"><div><h2 className="text-lg font-semibold">Deleted jobs</h2><p className="text-sm text-muted-foreground">These records remain available to MCP for duplicate checking.</p></div><div className="divide-y overflow-hidden rounded-lg border">{jobs.map((job) => <div key={job.id} className="flex flex-col gap-3 p-4 lg:flex-row lg:items-center"><div className="min-w-0 flex-1"><p className="truncate font-medium">{job.position} <span className="font-normal text-muted-foreground">at {job.company}</span></p><p className="text-sm text-muted-foreground">{job.role_type === "internship" ? "Internship" : "Full time"}{job.location ? ` · ${job.location}` : ""} · Added {formatDate(job.added_date)}</p></div><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" disabled={pendingId === job.id} onClick={() => moveJob(job.id, "jobs_found")}><RotateCcw />New</Button><Button size="sm" variant="outline" disabled={pendingId === job.id} onClick={() => moveJob(job.id, "jobs_snoozed")}><Clock3 />Snooze</Button><Button size="sm" disabled={pendingId === job.id} onClick={() => moveJob(job.id, "to_apply")}><Check />Move to Apply</Button>{job.url && <Button size="sm" variant="ghost" asChild><a href={job.url} target="_blank" rel="noreferrer">View<ArrowUpRight /></a></Button>}</div></div>)}</div></section>
}
